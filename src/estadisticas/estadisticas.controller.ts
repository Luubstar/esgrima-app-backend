...
export class EstadisticasController {
  ...
  async findAll(@Req() request: Request, @Res() res : Response) {
    return res.status(HttpStatus.OK).send(await this.estadisticasService.findAll(request));
  }
  ...
  async findOne(@Param('id') id: string,@Res() res : Response) {
    return res.status(HttpStatus.OK).send(await this.estadisticasService.findOne(id));
  }

  @ApiOkResponse({description:"La estadistica (si la encuentra)", type:Estadisticas})
  @ApiOperation({summary: "Encuentra una estadística dado un usuario, mes y año"})
  @Get(':userID/:month/:year')
  async GetByUser(@Param('userID') id: string,@Param("month") mes:number, @Param("year") año:number,@Res() res : Response) {return res.status(HttpStatus.OK).send(await this.estadisticasService.getFromUser(id,mes,año));}
}